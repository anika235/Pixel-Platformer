...
class Game:
    ...
    def load_platforms(self, filename):
        ...
        with open(filename, 'r') as file:
            for line in file:
                ...
                move = bool(int(parts[7])) if len(parts) > 7 else False  # Read the move flag from the file, default to False
                direction = int(parts[8]) if len(parts) > 8 else 1  # Read the direction from the file, default to 1 (right)
                move_distance = int(parts[9]) if len(parts) > 9 else 10  # Read the move distance from the file, default to 10
    ...
    def update_platform_positions(self):
        for platform in self.platforms:
            if platform['move']:  # Only move platforms with the move flag set to True
    ...
    def apply_physics(self):
        ...
        if self.is_on_platform and self.current_platform and self.current_platform['velocity'] != 0:
            # print(self.current_platform['velocity'])
            if self.current_platform['velocity'] > 0:
                self.char_x += 2
            else:
                self.char_x -= 2
            # self.char_x += self.current_platform['velocity'] + 1
        ...
        # Print the current position of the character
        print(f"Character Position: x={self.char_x}, y={self.char_y}")
